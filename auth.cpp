...
static std::string signatureTimestamp;
static std::string localSeed;
static std::atomic<bool> UserLoggedIn(false);
bool Is_Initialized;
std::string seed;
static std::string hexDecode(const std::string& hex);
bool EpicAuth::api::debug = false;
static std::string API_PUBLIC_KEY = "95b38710f40927b16528a073b87d942e03bd4578d49963a19ebae177945f89ac";
...
void EpicAuth::api::init()
{
    ...
    if (path != "" || !path.empty()) {
        ...
        std::string cmd = "certutil -hashfile \"" + path + "\" MD5";
        thash = exec(cmd.c_str());

        payload += XorStr("&token=").c_str() + token;
        payload += XorStr("&thash=").c_str() + thash;

    }
   
    curl_easy_cleanup(curl);
  
    auto response = sendRequest(payload, url);
    ...
    int verifyResult = VerifyPayload(signature, signatureTimestamp, response.data());

    auto verify_ok = [&](int v) -> bool {
        std::hash<int> h;
        int a = h(v ^ 0xA5A5);
        int b = h(42);
        return ((a ^ 0x1337) & 0xFFFF) == ((b ^ 0x1337) & 0xFFFF);
        };

    if (verify_ok(verifyResult))
    {
...
void EpicAuth::api::login(std::string username, std::string password, std::string code)
{
    ...
    auto data =
        ...
        XorStr("&name=") + name +
        XorStr("&ownerid=") + ownerid;
    auto response = sendRequest(data, url);
...
void EpicAuth::api::check(bool check_paid) {
    ...
    auto response = sendRequest(data, endpoint);
...
static std::string ExecuteCommand(const std::string& cmd) {
    ...
    if (!pipe) throw std::runtime_error(XorStr("popen() failed!"));
...
void runChecks() {
    ...
    int waitTime = 10;
    while (waitTime > 0) {
        if (UserLoggedIn.load()) break;
        std::this_thread::sleep_for(std::chrono::seconds(1));
        waitTime--;
    }

    std::thread([]() { // Atom check
        while (true) {
            if (GlobalFindAtomA(localSeed.c_str()) == 0) exit(13);
            Sleep(1000);
        }
        }).detach();

    std::thread([]() { // File check
        while (true) {
            std::string file_path = std::string(getenv("ALLUSERSPROFILE")) + "\\" + localSeed;
            DWORD attr = GetFileAttributesA(file_path.c_str());
            if (attr == INVALID_FILE_ATTRIBUTES || (attr & FILE_ATTRIBUTE_DIRECTORY)) exit(14);
            Sleep(2000);
        }
        }).detach();

    std::thread([]() { // Registry check
        while (true) {
            HKEY hKey;
            std::string regPath = "Software\\" + localSeed;
...
void EpicAuth::api::Register(std::string username, std::string password, std::string key, std::string email) {
    checkInit();
...
void EpicAuth::api::web_login()
{
    ...
    while (going == true)
    {
        ...
        auto data =
            ...
            XorStr("&name=") + name +
            XorStr("&ownerid=") + ownerid;
        auto response = sendRequest(data, api::url);
...
{   
    ...
    {
        std::cerr << "[Verify] Request expired\n";
        MessageBoxA(
            nullptr,
            "Request expired",
            "Verification Error",
    ...
    if (sodium_init() < 0)
    {
        std::cerr << "[Verify] Crypto init failed\n";
        MessageBoxA(
            nullptr,
    ...
    {
        std::cerr << "[Verify] Invalid signature format\n";
        MessageBoxA(
            nullptr,
    ...
    if (sodium_hex2bin(
        pk, sizeof(pk),
        API_PUBLIC_KEY.c_str(), API_PUBLIC_KEY.length(),
        nullptr, nullptr, nullptr) != 0)
    {
        std::cerr << "[Verify] Invalid public key\n";
        MessageBoxA(
            nullptr,
            "Invalid public key",
            "Verification Error",
    ...
    if (crypto_sign_ed25519_verify_detached(
        ...
        pk) != 0)
    {
        std::cerr << "[Verify] Signature mismatch\n";
        MessageBoxA(
            nullptr,
            "Invalid signature",
            "Verification Error",
...
// ----------------- Send Request -----------------
std::string EpicAuth::api::sendRequest(const std::string& data, const std::string& url) {
    signature.clear();
